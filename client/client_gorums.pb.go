...
func (c *Configuration) execCommandRecv(ctx context.Context, in *Command, msgID uint64, expected int, replyChan chan *orderingResult, fut *FutureEmpty) {
	...
	for {
		...
		case r := <-replyChan:
			...
			data := r.reply.(*Empty)
			replies[r.nid] = data
			if reply, quorum = c.qspec.ExecCommandQF(in, replies); quorum {
...
// RoundTrip asynchronously invokes a quorum call on configuration c
// and returns a FutureEmpty, which can be used to inspect the quorum call
// reply and error when available.
func (c *Configuration) RoundTrip(ctx context.Context, in *Empty) *FutureEmpty {
	fut := &FutureEmpty{
		NodeIDs: make([]uint32, 0, c.n),
		c:       make(chan struct{}, 1),
	}
	...
	go c.roundTripRecv(ctx, in, msgID, expected, replyChan, fut)

	return fut
}

func (c *Configuration) roundTripRecv(ctx context.Context, in *Empty, msgID uint64, expected int, replyChan chan *orderingResult, fut *FutureEmpty) {
	defer close(fut.c)
	...
	for {
		...
		case r := <-replyChan:
			...
			data := r.reply.(*Empty)
			replies[r.nid] = data
			if reply, quorum = c.qspec.RoundTripQF(in, replies); quorum {
				fut.Empty, fut.err = reply, nil
				return
			}
		case <-ctx.Done():
...
type QuorumSpec interface {
	...
	// RoundTripQF is the quorum function for the RoundTrip
	// asynchronous ordered quorum call method. The in parameter is the request object
	// supplied to the RoundTrip method at call time, and may or may not
	// be used by the quorum function. If the in parameter is not needed
	// you should implement your quorum function with '_ *Empty'.
	RoundTripQF(in *Empty, replies map[uint32]*Empty) (*Empty, bool)
}
...
type Client interface {
	...
	RoundTrip(context.Context, *Empty, func(*Empty, error))
}
...
func (s *GorumsServer) RegisterClientServer(srv Client) {
	...
	s.srv.handlers[roundTripMethodID] = func(ctx context.Context, in *gorumsMessage, finished chan<- *gorumsMessage) {
		req := in.message.(*Empty)
		once := new(sync.Once)
		f := func(resp *Empty, err error) {
			once.Do(func() {
...
const roundTripMethodID int32 = 2
...
var orderingMethods = map[int32]methodInfo{
	...
	2: {requestType: new(Empty).ProtoReflect(), responseType: new(Empty).ProtoReflect()},
}
